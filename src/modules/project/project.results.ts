import type { DepreciationAsset, Project, User } from "@prisma/client"

export type ProjectResults = {
  // Invest
  investmentTotal: number

  // Revenus / charges / prêt
...
function safePct(p: number | null | undefined) {
  return (p ?? 0) / 100
}

function annualizeMonthly(m: number) {
  return m * 12
}

function computeAnnualDepreciation(assets: DepreciationAsset[] | undefined) {
 ...
 */
export function computeProjectResults(
  project: Project & { depreciationAssets?: DepreciationAsset[]; user?: Pick<User, "tmi" | "socialContribRate"> }
): ProjectResults {
