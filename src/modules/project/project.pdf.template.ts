...
import type { ProjectResults } from "./project.results"
...
export function renderProjectPdfHtml(project: any, results?: ProjectResults, opts: PdfOptions = {}) {
  ...
  const depreciationAssets = (project.depreciationAssets ?? []) as Array<{
    id: string
    category: string
    label: string
    amount: number
  ...
  <div class="page">
    <div class="paper">
      ...
      <div class="section">
        ...
        ${detailLine("Gestion locative (calculée)", mask(eur((results as any).managementMonthly ?? 0), true))}
...
function detailLine(label: string, value: string, highlight = false) {
  return `
